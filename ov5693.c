...
/* camera parameters are declared in this structure */
struct st* camera_parameters(void);

/* here global structure variable created which is used for application */
struct st cam = { };

/* camera parameters are declared in this structure */
struct ov5693_mode extra;
...
/* this function is used to get camera parameters through cam variable */
struct st* camera_parameters(void)
{
...
/* This is used to print data through proc */

static int ov5693_proc_show(struct seq_file *m, void *v)
{

        seq_printf(m, "******** ov5693 camera parameters**********\n");
        seq_printf(m, "Parameter name:          parameter values     \n");
        seq_printf(m, "mode of camera         :  %d\n ", cam.default_mode);
        seq_printf(m, "picture width          :  %d\n ", cam.default_width);
        seq_printf(m, "picture height         :  %d\n ", cam.default_height);
        seq_printf(m, "picture fmt_width      :  %d\n ", cam.fmt_width);
        seq_printf(m, "picture fmt_height     :  %d\n ", cam.fmt_height);
        seq_printf(m, "picture numctrls       :  %d\n ", cam.numctrls);
        seq_printf(m, "picture numfmts        :  %d\n ", cam.numfmts);
        seq_printf(m, "clkfrequency           :  %d\n ", cam.def_clk_freq);
        seq_printf(m, "picture frame length   :  %d \n ", cam.frame_length);
        seq_printf(m, "gain                   :  %d \n ", cam.gain);
        seq_printf(m, "picture coarse time    :  %d \n ", cam.coarse_time);

        return 0;
}

static void *ov5693_proc_start(struct seq_file *m, loff_t *pos)
{
        return *pos < 1 ? (void *)1 : NULL;
}

static void *ov5693_proc_next(struct seq_file *m, void *v, loff_t *pos)
{
        ++*pos;
        return NULL;
}

static void ov5693_proc_stop(struct seq_file *m, void *v)
{
}

const struct seq_operations ov5693_proc_op = {
        .start  = ov5693_proc_start,
        .next   = ov5693_proc_next,
        .stop   = ov5693_proc_stop,
        .show   = ov5693_proc_show
};
			...
			const struct i2c_device_id *id)
{
	...
	cam.default_mode                = common_data->def_mode;
        cam.default_height              = common_data->def_height;
        cam.default_width               = common_data->def_width;
        cam.fmt_width                   = common_data->fmt_width;
        cam.fmt_height                  = common_data->fmt_height;
        cam.numctrls                    = common_data->numctrls;
        cam.numfmts                     = common_data->numfmts;
        common_data->def_clk_freq       = OV5693_DEFAULT_CLK_FREQ;
        cam.def_clk_freq                = common_data->def_clk_freq;
        extra.frame_length              =  OV5693_DEFAULT_FRAME_LENGTH;
        cam.frame_length                = extra.frame_length;
        extra.gain                      = OV5693_DEFAULT_GAIN;
        cam.gain                        = extra.gain;
        extra.coarse_time               = OV5693_MIN_EXPOSURE_COARSE;
        cam.coarse_time                 = extra.coarse_time;
